...
async function main () {
  ...
  const ids = []
  for (const file of await glob(`${__dirname}/src/**/*.js`)) {
    const id = rndid(10, 'aA')
    ids.push(id)
    out += `import * as ${id} from '${file.replace(`${__dirname}/src/`, './src/')}'\n`
  }
  const typedefs = mergeTypeDefs(loadFilesSync(`${__dirname}/src/*.graphql`, { recursive: true }))
